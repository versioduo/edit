...
class V2MIDIFileTrack {
  ...
  parse(buffer) {
    ...
    const parseSystemExclusive = (event) => {
      ...
      const [size, sizeLength] = decodeNumber(cursor);
      cursor += sizeLength;
      event.data = new Uint8Array(this.#buffer, cursor, size);      cursor += size;
    }
...
class V2MIDIFilePlayer extends V2MIDIFile {
  ...
  play(handler) {
    ...
    if (this.#tick == 0) {
      ...
      // 120 BPM default == 0.5 sec beat.
      this.#beatSec = 1 / (120 / 60);
  ...
  #timerHandler() {
    ...
    const passedSec = nowSec - this.#lastSec;
    this.#lastSec = nowSec;

    // The number of beats since the last run.
    const beats = passedSec / this.#beatSec;

    // Add the number ticks which have passed.
    this.#tick += beats * this.division;
    ...
    for (const [i, track] of this.tracks.entries()) {
      ...
      if (event.meta == V2MIDIFile.Meta.tempo)
        this.#beatSec = event.getTempo() / 1000000;
